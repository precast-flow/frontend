...
export function PrePourReinforcementCheckRow({
  ...
  const { t } = useI18n()
  const { scanRebarMaterial, removeRebarScan } = useWorkQueue()
  ...
  const handleScan = (raw: string) => {
    const result = scanRebarMaterial(item.id, raw, item)
              ...
              <ul className="space-y-1.5">
                {scans.map((scan) => {
                  ...
                  return (
                    <li
                      key={scan.scanId}
                      className="flex items-start justify-between gap-2 rounded-md bg-white/80 px-2 py-1.5 text-xs dark:bg-slate-800/80"
                    >
                      ...
                      <button
                        type="button"
                        className="shrink-0 rounded p-1 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40"
                        aria-label={t('qualityQrScan.remove')}
                        onClick={() => removeRebarScan(item.id, scan.scanId)}
                      >
                        <X className="size-4" />
                      </button>
                    </li>
