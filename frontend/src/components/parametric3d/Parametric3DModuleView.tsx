  ...
  return (
    <label className="flex flex-col gap-1">
      ...
      <input
        ...
        className="okan-liquid-input border-0 px-3 py-2 text-sm shadow-none focus:outline-none disabled:opacity-50"
      />
    </label>
  )
}
      ...
      <div className="okan-liquid-content flex min-h-0 flex-1 flex-col gap-4">
        ...
        <div className={`${panelClass} flex flex-col gap-4 p-4`}>
          ...
          <div className="grid gap-3">
            <label className="flex flex-col gap-1">
              ...
              <input
                ...
                className="okan-liquid-input border-0 px-3 py-2 text-sm shadow-none focus:outline-none"
              />
            </label>
            ...
            {family === 'CULVERT' ? (
              <>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('parametric3d.culvert.outer')}</p>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('parametric3d.culvert.inner')}</p>
            ...
            {family === 'PANEL' ? (
              <>
                <label className="flex flex-col gap-1">
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('parametric3d.panelKind')}</span>
                  <select
                    ...
                    className="okan-liquid-select border-0 px-3 py-2 text-sm shadow-none"
                  >
            ...
            {family === 'PROFILE_WALL' ? (
              <>
                <label className="flex flex-col gap-1">
                  <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('parametric3d.profileType')}</span>
                  <select
          ...
          <div className="border-t border-white/20 pt-3 dark:border-white/10">
            <label className="flex flex-col gap-1">
              <span className="text-xs text-slate-500 dark:text-slate-400">{t('parametric3d.saveAs')}</span>
              <input
                value={nameDraft}
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button type="button" onClick={handleSave} className="okan-liquid-btn-primary px-4 py-2 text-sm font-semibold">
                {t('parametric3d.save')}
              </button>
              <button type="button" onClick={exportGlb} className="okan-liquid-btn-secondary px-4 py-2 text-sm font-semibold">
            ...
            {errorMsg ? (
              <p
                role="status"
                className="okan-liquid-banner-warn mt-2 px-3 py-2 text-xs text-amber-950 dark:text-amber-100"
              >
                {errorMsg}
              </p>
            ) : null}
        ...
        <div className="flex min-h-[420px] flex-col gap-3">
          <div className={`${panelClass} flex min-h-[360px] flex-1 flex-col p-3`}>
            {showViewer ? (
              <ParametricViewer payload={deferredPayload} />
            ) : (
              <div
                className={`${nestedClass} flex h-full min-h-[320px] items-center justify-center p-6 text-center text-sm text-slate-600 dark:text-slate-300`}
              >
          ...
          <div className={`${panelClass} p-4`}>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">{t('parametric3d.savedTitle')}</h3>
            <ul className="mt-2 max-h-48 space-y-2 overflow-auto text-sm">
              {saved.length === 0 ? (
                <li className="text-slate-500 dark:text-slate-400">{t('parametric3d.savedEmpty')}</li>
              ) : (
                saved.map((s) => (
                  <li key={s.id} className="okan-liquid-list-card flex items-center justify-between gap-2 px-3 py-2">
                    <button
                      type="button"
                      onClick={() => loadDesign(s)}
                      className="min-w-0 flex-1 truncate text-left font-medium text-slate-800 underline-offset-2 hover:underline dark:text-slate-100"
                    >
                      {s.name}
                    </button>
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-lg px-2 py-1 text-xs text-red-600 hover:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-950/40"
                    >
