...
type Props = {
  ...
  children: ReactNode
  actions?: ReactNode
  /** Sayfa konumu (örn. Hesap › Profil) — başlığın altında */
 ...
 */
export function ModuleShellFrame({ title, children, actions, breadcrumb }: Props) {
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded-3xl bg-pf-surface p-5 shadow-neo-out md:p-6">
      <div className="mb-5 border-b border-gray-200/90 pb-4 dark:border-gray-700/90">
        ...
        {breadcrumb ? <div className="mt-1.5">{breadcrumb}</div> : null}
      </div>
      {children}
    </div>
  )
