...
type GeneralPlanningContextValue = {
  ...
  planHistory: { checkpoints: GeneralPlanCheckpoint[]; cursor: number }
  setPlanHistory: React.Dispatch<
    React.SetStateAction<{ checkpoints: GeneralPlanCheckpoint[]; cursor: number }>
  >
...
export function GeneralPlanningProvider({
  ...
  const appendCheckpoint = useCallback((nextItems: GeneralPlanItem[], label: string, note?: string) => {
