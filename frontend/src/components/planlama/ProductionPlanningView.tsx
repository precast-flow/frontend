...
function ProductionPlanningInner() {
  ...
  const canView = access.hasPermission(UNIT_VIEW_PERMISSIONS.production)

  if (!canView) {
    return (
      <div className="flex flex-1 items-center justify-center p-8 text-center text-sm text-slate-600 dark:text-slate-400">
        Üretim planlama için yetkiniz bulunmuyor.
      </div>
    )
  }

  return <PlanningTimelineView variant="production" />
}

/** Genel planlama — Üretim birimi görünümü (üretim modülü altında ayrı sayfa). */
export function ProductionPlanningView() {
  return (
    <GeneralPlanningProvider lockUnit="production">
