...
export type PlanningItemProgressDensity = 'full' | 'compact' | 'minimal'

type Props = {
  stage: PlanningProductionStage
  stageProgress?: number
  density?: PlanningItemProgressDensity
}

export function PlanningItemProgress({
  stage,
  stageProgress = 50,
  density = 'full',
}: Props) {
  ...
  if (density === 'minimal') {
    return (
      ...
      >
        <div
          className="h-1 min-w-[1.25rem] flex-1 overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-700/60"
          role="progressbar"
  ...
  if (density === 'compact') {
    return (
      <div className="flex min-w-0 shrink-0 flex-col gap-0.5" title={title}>
        <span className="truncate text-[8px] font-medium leading-none text-slate-600 dark:text-slate-400">
          {label}
        </span>
        <div
          className="h-1 w-full min-w-[1.5rem] overflow-hidden rounded-full bg-slate-200/80 dark:bg-slate-700/60"
          role="progressbar"
          aria-valuenow={percent}
          aria-valuemin={0}
          aria-valuemax={100}
