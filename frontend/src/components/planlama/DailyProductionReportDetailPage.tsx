...
import type { DailyProductionReportDetailLocationState } from '../../data/dailyProductionReportPaths'
import { DailyProductionReportView } from './DailyProductionReportView'
import '../muhendislikOkan/engineeringOkanLiquid.css'
import '../proje/projectManagementGlassLight.css'

export function DailyProductionReportDetailPage() {
  const { t } = useI18n()
  ...
  const { getDailyProductionReport } = useWorkQueue()

  const navState = location.state as DailyProductionReportDetailLocationState | null
  const report = reportId ? getDailyProductionReport(reportId) : undefined

  if (!report) {
    return <Navigate to={moduleIdToPath('production-planning')} replace />
  }
  ...
  const goBack = () => {
    ...
    navigate(moduleIdToPath('production-planning'))
  }
      ...
      <div className="flex flex-col gap-1">
        <div className="px-[0.6875rem] pt-0 pb-0.5">
          <div className="mb-1.5 pb-1.5">
            ...
            >
              {t('productionPlanning.dailyReport.detailPageTitle', { reportNo: report.reportNo })}
            </h1>
          </div>
        ...
        <DailyProductionReportView report={report} gl={gl} onBack={goBack} layout="page" />
      </div>
    </div>
  )
}
